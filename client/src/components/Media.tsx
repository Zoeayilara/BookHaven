...
const convertGoogleDriveUrl = (url: string): string => {
  // Clean up the URL (remove trailing spaces)
...
const convertGooglePhotosUrl = (url: string): string => {
  // For Google Photos, we need to use the uc export format similar to Google Drive
  // Extract the share ID from the URL
  const match = url.match(/photos\.app\.goo\.gl\/([a-zA-Z0-9]+)/);
  if (match && match[1]) {
    // Use Google's lh3 CDN which serves Google Photos images
    // This format works for direct image loading
    return `https://photos.app.goo.gl/${match[1]}=w800`;
  }
  return url;
};
...
export default function Media() {
  ...
  useEffect(() => {
    ...
    fetch("/media-content.json")
      .then((res) => res.json())
      .then((data) => {
        // Convert Google Drive and Google Photos URLs to direct image URLs
        const processedData = {
          ...data,
          images: data.images.map((img: Image) => ({
            ...img,
            displayUrl: img.url.includes("drive.google.com") 
              ? convertGoogleDriveUrl(img.url)
              : img.url.includes("photos.app.goo.gl")
              ? convertGooglePhotosUrl(img.url)
              : img.url
          }))
        };
                  ...
                  >
                    <div className="aspect-square overflow-hidden bg-muted">
                      <img
                        ...
                        onError={(e) => {
                          console.error("Image failed to load:", image.title, e.currentTarget.src);
                          // Try alternative Google Drive format
                          const target = e.target as HTMLImageElement;
                          if (image.url.includes("drive.google.com")) {
                            const fileId = image.url.match(/\/d\/([a-zA-Z0-9-_]+)/);
                            if (fileId && fileId[1]) {
                              // Try different Google Drive formats
                              const alternativeUrl = `https://lh3.googleusercontent.com/d/${fileId[1]}=s800`;
